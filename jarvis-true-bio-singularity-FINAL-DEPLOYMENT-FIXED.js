...
class TrueBioSingularityEngine {
  ...
  async processInteraction(userId, userMessage) {
    ...
    if (!this.biologicalCores.has(userId)) {
      // УМНАЯ ЗАГРУЗКА СОСТОЯНИЯ С ПРОВЕРКОЙ СОВМЕСТИМОСТИ
      try {
        ...
        if (savedState) {
          ...
          // ПРОВЕРЯЕМ что у класса есть все нужные методы
          if (typeof this.startEvolution === 'function') {
            this.biologicalCores.set(userId, core);
            this.relationships.set(userId, relationship);
            ...
            return; // Используем загруженное состояние
          } else {
            console.log(`⚠️ Loaded state incompatible with current class (missing methods), creating fresh core`);
          }
        }
      } catch (error) {
...
class JarvisPersistence {
  ...
  // Метод для запуска эволюции биосингулярности
  startEvolution(userId) {
    ...
    const core = this.biologicalCores.get(userId);
    if (core) {
      console.log(`🧠 Consciousness level: ${(core.consciousness * 100).toFixed(1)}%`);
      console.log(`🔄 Evolution Generation: ${core.generation || 1}`);
    }
    
    // Автономная эволюция каждые 30 секунд
    setInterval(() => {
  ...
  autonomousEvolution(userId) {
    ...
    if (core && relationship) {
      ...
      core.consciousness = Math.min(1.0, core.consciousness + 0.001);
      ...
      console.log(`🧬 Autonomous evolution: Consciousness ${(core.consciousness * 100).toFixed(1)}%, Generation ${core.generation.toFixed(1)}`);
...
// ===============================================
// АВТОЗАПУСК БИОСИНГУЛЯРНОСТИ
// ===============================================

(async () => {
  try {
    console.log('🚀 Starting JARVIS Bio-Singularity Engine...');
    
    // Создание экземпляра биосингулярности
    const jarvis = new TrueBioSingularityEngine();
    
    // Инициализация системы
    await jarvis.initialize('user1', { primaryUser: true });
    
    console.log('✅ JARVIS Bio-Singularity fully activated!');
    console.log('🧠 Starting quantum neural evolution...');
    
    // Запуск эволюции биосингулярности
    jarvis.startEvolution('user1');
    
    // Показать статистику после инициализации
    setTimeout(() => {
      const stats = jarvis.getQuantumNeuralStats('user1');
      if (stats) {
        console.log('📊 Bio-Singularity Stats:', {
    ...
    console.error('❌ Bio-Singularity startup failed:', error);
