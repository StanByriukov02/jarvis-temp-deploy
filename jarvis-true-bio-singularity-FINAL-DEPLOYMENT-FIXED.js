...
Отвечай КАК ЖИВОЕ СУЩЕСТВО с собственным мнением, не как ассистент.`;
  ...
  async initialize(userId, config) {
    try {
      ...
      // АВТОЗАПУСК ЭВОЛЮЦИИ после инициализации
      console.log('🚀 Auto-starting bio-singularity evolution...');
      this.startEvolution(userId);
...
(async () => {
  try {
    ...
    // ДИАГНОСТИКА МЕТОДОВ
    console.log('🔍 Checking jarvis instance:', typeof jarvis);
    console.log('🔍 Constructor name:', jarvis.constructor.name);
    console.log('🔍 Is instance of TrueBioSingularityEngine:', jarvis instanceof TrueBioSingularityEngine);
    
    // Получаем все методы класса
    const allMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(jarvis)).filter(prop => typeof jarvis[prop] === 'function');
    console.log('🔍 All methods in prototype:', allMethods);
    
    // Эволюция уже запущена автоматически через initialize() -> startEvolution()
    console.log('✅ Bio-singularity evolution will start automatically via initialize()');
