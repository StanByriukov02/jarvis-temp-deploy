...
(async () => {
  try {
    ...
    // Запуск эволюции ПОСЛЕ полной инициализации с небольшой задержкой
    setTimeout(() => {
      console.log('🚀 Starting bio-singularity evolution after initialization...');
      if (typeof jarvis.startEvolution === 'function') {
        console.log('✅ startEvolution method found, launching...');
        jarvis.startEvolution('user1');
      } else {
        console.log('❌ startEvolution method not found!');
      }
    }, 1000); // 1 секунда задержки для завершения инициализации
