...
function startContinuousEvolution(jarvis, userId) {
  ...
  evolutionTimer = setInterval(() => {
    try {
      const core = jarvis.users.get(userId);
      const relationship = jarvis.relationships.get(userId);
...
(async () => {
  try {
    ...
    setTimeout(() => {
      startContinuousEvolution(globalJarvisInstance, 'user1');
    }, 2000);
